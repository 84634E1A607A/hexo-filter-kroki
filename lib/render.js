 ...
 */
function serverSideRendering(pluginConfig, encodedUrl, darkMode) {
                ...
                httpReq.get(encodedUrl, response => {
                    ...
                    response.on('data', function (chunk) {
                        ...
                        if (pluginConfig.link === "inlineBase64") {
                            resolve(`<img class="${pluginConfig.className} ${darkMode ? pluginConfig.darkClassName : ""}" src='data:image/svg+xml;base64,${buffer.toString("base64")}'>`);
                        } else if (pluginConfig.link === "inlineUrlEncode") {
                            resolve(`<img class="${pluginConfig.className} ${darkMode ? pluginConfig.darkClassName : ""}" src='data:image/svg+xml;utf8,${encodeURIComponent(buffer.toString())}'>`);
                        } else {
                            var x = svgAddClass(buffer.toString(),pluginConfig.className);
                            if (darkMode) x = svgAddClass(x, pluginConfig.darkClassName);
                            resolve(x);
                        }
                    });
                });
